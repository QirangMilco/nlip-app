...
pub fn get_macos_text() -> Result<String, NlipError> {
...
fn get_selection_by_accessibility() -> Result<Selection, NlipError> {
    ...
    let selected_text = match text_value.downcast_into::<CFString>() {
        Some(text) => text,
        None => return Err(NlipError::NoSelectedContent),
    };
...
fn get_selection_by_clipboard() -> Result<Selection, NlipError> {
...
"#;
    ...
    if !output.status.success() {
        ...
        return Err(NlipError::AppleScriptError(stderr.to_string()));
    }
